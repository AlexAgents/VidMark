...
Automatically finds dependencies, generates icon and runs PyInstaller.
...
def default_ui_mode() -> str:
    """
    Choose default UI mode for PyInstaller based on platform.
    On Windows, GUI apps should be --windowed by default (no console window).
    """
...
def resolve_ui_mode(args) -> str:
    """
    Resolve UI mode based on CLI flags.
    Priority: explicit flag > debug > platform default.
    """
        ...
        try:
            ...
            if num_layouts > 0:
                ...
                for lid in layout_ids:
                    lang_id = lid & 0xFFFF if lid else 0
                    layouts.append(lang_id)
                if 0x0419 in layouts:
                    return "ru"
        except Exception:
            pass
...
def find_entry_point() -> Optional[str]:
    """Find entry point (main.py or __main__.py)."""
...
def check_pyinstaller() -> bool:
    """Check PyInstaller availability and offer installation."""
...
def generate_icon_pillow(path: str) -> bool:
    """Generate icon using Pillow library."""
    try:
    ...
    try:
        ...
        draw.rounded_rectangle(rect_coords, radius=40, fill=ICON_COLOR)
...
def generate_icon_fallback(path: str) -> bool:
    """Create simple valid ICO stub without Pillow."""
    ...
    try:
        ...
        print_error(f"Fallback icon generation error: {e}")
        return False
...
def scan_hidden_imports() -> List[str]:
    ...
    stdlib_skip = {
        ...
        # Local packages (PyInstaller finds these via --paths)
...
def build_exe(args=None) -> bool:
    ...
    # Verify style.qss exists (warn if missing)
    style_qss = os.path.join(ASSETS_DIR, "style.qss")
    if not os.path.exists(style_qss):
        print_warn(f"assets/style.qss not found — app will use default Qt styling")
    ...
    # Collect package data for libraries that store runtime files
    ...
    icon_status = t("yes") if os.path.exists(ICON_PATH) else t("no")
    print(f"🖼️  {t('icon_label')}: {icon_status}")
...
def interactive_menu():
    ...
    while True:
        ...
        print(f" 3. 🧹 {t('opt_clean_build')}")
        print(f" 4. 🧹 {t('opt_clean_dist')}")
        print(f" 5. 🗑  {t('opt_clean_all')}")
        print(f" 6. 🌐 {t('opt_change_lang')}")
...
def main():
    ...
    parser.add_argument("--debug", action="store_true",
                        help="Debug build: forces console mode")
    ui_group = parser.add_mutually_exclusive_group()
    ui_group.add_argument("--console", action="store_true",
                          help="Force console mode")
    ui_group.add_argument("--windowed", action="store_true",
                          help="Force windowed mode (no console)")
